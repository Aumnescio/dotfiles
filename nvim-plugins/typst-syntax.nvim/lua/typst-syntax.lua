...
-- |> Core functionality
function Module.create_typst_matchgroups()
    vim.cmd([[
        " Vim syntax file
        " Language: Typst
    ...
    -- |> Headings              ( STATE: Good )
    -- syn({ cmd = "syntax", args = { "match", "TypstNvimMarkupHeading",
    --     [[/^\s*\zs=\{1,6}\s.*$/]]
    -- }}, {})
    --
    -- -- |> Unordered List        ( STATE: Good )
    -- syn({ cmd = "syntax", args = { "match", "TypstNvimMarkupUnorderedList",
    --     [[/\v^\s*-\s+/]]
    -- }}, {})
    --
    -- -- |> Ordered List          ( STATE: Good )
    -- syn({ cmd = "syntax", args = { "match", "TypstNvimMarkupOrderedList",
    --     [[/\v^\s*(\+|\d+\.)\s+/]]
    -- }}, {})

    -- -- |> Code                  ( STATE: Testing )
    -- syn({ cmd = "syntax", args = { "cluster", "TypstNvimCode",
    --     [[contains=@TypstNvimCodeKeyword,@TypstNvimCodeConstant,@TypstNvimCodeIdentifier,@TypstNvimCodeFunction,@TypstNvimCodeParens]]
    -- }}, {})
    --
    -- -- |> Code Region           ( STATE: Testing )
    -- syn({ cmd = "syntax", args = { "region", "TypstNvimCodeRegion", "contained",
    --     [[matchgroup=TypstNvimCodeStatement start=/\v(let|set|show|import|include)>-@!/ end=/\v%(;|$)/]],
    --     [[contains=@TypstNvimCode]]
    -- }}, {})
    --
    -- -- |> Code Keywords         ( STATE: Testing )
    -- syn({ cmd = "syntax", args = { "keyword", "TypstNvimCodeKeyword", "contained",
    --     [[if else while for not in and or return]]
    -- }}, {})
    --
    -- -- |> Code Identifiers      ( STATE: Testing )
    -- syn({ cmd = "syntax", args = { "match", "TypstNvimCodeIdentifier", "contained",
    --     [[/\v\K\k*%(-+\k+)*>-@!(<%(let|set|show|import|include))@<![\.\[\(]@!/]]
    -- }}, {})
end
...
function Module.setup(opts)
    ...
    vim.api.nvim_create_autocmd("FileType", {
        pattern = { "typst" },
        callback = function()
            Module.create_typst_matchgroups()
        end,
    })
end
...
--     -- |> Set Base Highlights
--     vim.cmd([[
--         highlight default link TypstNvimMarkupHeading                   Warning
--         highlight default link TypstNvimMarkupUnorderedList             Warning
--         highlight default link TypstNvimMarkupOrderedList               Warning
--         highlight default link TypstNvimKeyword                         Error
--         highlight default link TypstNvimCode                            Error
--         highlight default link TypstNvimCodeRegion                      Error
--         highlight default link TypstNvimCodeKeyword                     Error
--         highlight default link TypstNvimCodeIdentifier                  Error
--     ]])
-- end
